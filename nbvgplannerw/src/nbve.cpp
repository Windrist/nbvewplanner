...
namespace nbvePlanner {
    ...
    Planner::Planner(const ros::NodeHandle& nh) : nh_(nh) {
        plannerService_ = nh_.advertiseService("nbveplanner", &Planner::plannerCallback, this);
        rePlannerService_ = nh_.advertiseService("nbvereplanner", &Planner::rePlannerCallback, this);
        ...
        comTPub_ = nh_.advertise<std_msgs::Float64>("/computationTime", 10);
    ...
    bool Planner::setParams() {
        ...
        if (!ros::param::get(ns + "/nbvep/gain/degressive_coeff", params_.degressiveCoeff_)) {
        ...
        if (!ros::param::get(ns + "/nbvep/tree/extension_range", params_.extensionRange_)) {
        ...
        if (!ros::param::get(ns + "/nbvep/tree/min_extension_range", params_.minExtensionRange_)) {
            ROS_WARN("No value for minimal extension range specified. Looking for %s. Default is 1.0m.",
                    (ns + "/nbvep/tree/min_extension_range").c_str());
        }
        params_.exact_root_ = true;
        if (!ros::param::get(ns + "/nbvep/tree/exact_root", params_.exact_root_)) {
            ROS_WARN("No option for exact root selection specified. Looking for %s. Default is true.",
                    (ns + "/nbvep/tree/exact_root").c_str());
        }
        params_.initIterations_ = 15;
        if (!ros::param::get(ns + "/nbvep/tree/initial_iterations", params_.initIterations_)) {
            ROS_WARN("No number of initial tree iterations specified. Looking for %s. Default is 15.",
                    (ns + "/nbvep/tree/initial_iterations").c_str());
        }
        params_.cuttoffIterations_ = 200;
        if (!ros::param::get(ns + "/nbvep/tree/cuttoff_iterations", params_.cuttoffIterations_)) {
        ...
        if (!ros::param::get(ns + "/nbvep/tree/number_samples", params_.numberSamples_)) {
            ROS_WARN("No cuttoff iterations value specified. Looking for %s. Default is 200.",
                    (ns + "/nbvep/tree/number_samples").c_str());
        }
        ...
        params_.miniBoundingBox_.x() = 0.1;
        if (!ros::param::get(ns + "/system/mnbx/x", params_.miniBoundingBox_.x())) {
            ROS_WARN("No x size Mini Bounding Box value specified. Looking for %s. Default is 0.1m.", (ns + "/system/mnbx/x").c_str());
        }
        params_.miniBoundingBox_.y() = 0.1;
        if (!ros::param::get(ns + "/system/mnbx/y", params_.miniBoundingBox_.y())) {
            ROS_WARN("No y size Mini Bounding Box value specified. Looking for %s. Default is 0.1m.", (ns + "/system/mnbx/y").c_str());
        }
        ...
        if (!ros::param::get(ns + "/nbvep/replan/range", paramsRePlanner_.replanRange)) {
            ROS_WARN("No cuttoff iterations value specified. Looking for %s. Default is 0.5.",
                    (ns + "/nbvep/replan/range").c_str());
        }
        paramsRePlanner_.sampleRange = 0.5;
        if (!ros::param::get(ns + "/nbvep/replan/sample_range", paramsRePlanner_.sampleRange)) {
            ROS_WARN("No cuttoff iterations value specified. Looking for %s. Default is 0.5.",
                    (ns + "/nbvep/replan/sample_range").c_str());
        }
        paramsRePlanner_.numberSamples = 200;
        if (!ros::param::get(ns + "/nbvep/replan/number_samples", paramsRePlanner_.numberSamples)) {
            ROS_WARN("No cuttoff iterations value for Replanner specified. Looking for %s. Default is 200.",
                    (ns + "/nbvep/replan/number_samples").c_str());
        }
    ...
    bool Planner::plannerCallback(nbvgplannerw::nbvep_srv::Request& req,
                                                          nbvgplannerw::nbvep_srv::Response& res) {
        ...
        std_msgs::Float64 comTime;
        comTime.data = (ros::Time::now() - computationTime).toSec();
        comTPub_.publish(comTime);
        ROS_INFO("Path computation lasted %2.3fs", comTime.data);
        return true;
    }

    bool Planner::rePlannerCallback(nbvgplannerw::nbvep_resrv::Request& req,
                                                          nbvgplannerw::nbvep_resrv::Response& res) {
        // tree_->slideMapWeight();
