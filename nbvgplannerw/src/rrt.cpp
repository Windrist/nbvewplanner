...
namespace nbvePlanner {
    ...
    double RRT::computeGain(Eigen::Vector3d state) {
        ...
        double clamping_thres_min = 0.25;
        double clamping_thres_max = 0.75;
        ...
        for (std::size_t i = 0; i < params_.numberSamples_; i++) {
            ...
            CollisionDetector::CellStatus betweenStatus = cd_.whatBetweenBoundingBox(origin, vec, params_.miniBoundingBox_);
            ...
            if (nodeStatus == CollisionDetector::CellStatus::kError) {
                ...
                if (checkVisibility) {
                    ...
                    if (px < clamping_thres_max) occupied_gain += pow(resolution_, 2.0) * params_.igOccupied_ * params_.igProbabilistic_ / exp(-2*(1 - 2*px));
                }
            }
            else {
                // if (checkVisibility) {
