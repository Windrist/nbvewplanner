...
namespace nbvePlanner {
    ...
    void RRT::initialize() {
        ...
        publishTreePoints(rootNode_->state, rviz_visual_tools::colors::BLUE);
        ...
        iter != bestBranchMemory_.rend(); ++iter) {
            ...
            bool checkFrontier = CollisionDetector::CellStatus::kUnknown == status;

            if (checkVisibility || checkFrontier) {
    ...
    double RRT::computeGain(Eigen::Vector3d state) {
        ...
        double clamping_thres_min = 0.0;
        double clamping_thres_max = 1.0;
        ...
        for (std::size_t i = 0; i < params_.numberSamples_; i++) {
            ...
            CollisionDetector::CellStatus betweenStatus = cd_.whatBetween(origin, vec);

            bool checkVisibility = (CollisionDetector::CellStatus::kOccupied != betweenStatus) && (CollisionDetector::CellStatus::kError != betweenStatus);
            if (nodeStatus == CollisionDetector::CellStatus::kError) {
                continue;
            }
            else if (nodeStatus == CollisionDetector::CellStatus::kUnknown) {
                if (checkVisibility) {
                    unmapped_gain += pow(resolution_, 2.0) * params_.igUnmapped_;
                }
            }
            else if (nodeStatus == CollisionDetector::CellStatus::kOccupied) {
                if (checkVisibility) {
                    // occupied_gain += pow(resolution_, 2.0) * params_.igOccupied_;
                    occupied_gain += pow(resolution_, 2.0) * params_.igOccupied_ * (params_.igProbabilistic_ / occupancy_thres) * (clamping_thres_max - (double) probability / 100.0);
                }
            }
            else {
                if (checkVisibility) {
                    // free_gain += pow(resolution_, 2.0) * params_.igFree_;
                    free_gain += pow(resolution_, 2.0) * params_.igFree_ * (params_.igProbabilistic_ / occupancy_thres) * (1.0 - (double) probability / 100.0 - clamping_thres_min);
                }
            }
        }
    ...
    std::vector<geometry_msgs::Pose> RRT::getBestEdge(std::string targetFrame) {
        ...
        if (current->parent != nullptr) {
            publishTreePoints(current->state, rviz_visual_tools::colors::RED);
            exact_root_ = current->state;
            ret.push_back(sampleGoal(exact_root_, targetFrame));
            history_.push(current->parent->state);
            while (current->parent != rootNode_ && current->parent != nullptr) {
                current = current->parent;
            }
        }
        return ret;
    ...
    void RRT::publishTreePoints(Eigen::Vector3d state, rviz_visual_tools::colors color) {
        params_.visualizeTreePoints_->publishSphere(Eigen::Vector3d(state.x(), state.y(), 0.0),
        color, rviz_visual_tools::XXXLARGE, "TreeSamplePoints");
    }
